...
namespace BL.BaseControl
{
    public class BaseControl
    {
            ...
            try
            {
                using (OdbcConnection connection = new OdbcConnection(DL.Connection.GetConnectionStringGen(mode)))
                {
                    ...
                    string query = $@"SELECT
                                    ...
                                    END AS estatus_ruta,
                                    CASE
	                                    WHEN B.estado = 'P' THEN B.estado||C.estado||'-Retenido o Transito'				
	                                    ...
	                                    WHEN B.estado = 'C' THEN B.estado||C.estado||'-PreCancelado'
	                                    WHEN B.estado = 'E' THEN B.estado||C.estado||'-Entregado'
	                                    WHEN B.estado = 'D' THEN B.estado||C.estado||'-Devuelto'
	                                    WHEN B.estado = 'G' THEN B.estado||C.estado||'-Generado Backorder'
	                                    WHEN B.estado = 'W' THEN B.estado||C.estado||'-Cerrado Por Inventario'
                                    ...
                                    CASE
	                                    ...
	                                    WHEN D.rt_stat = 5 THEN '5-Evento sin regiistrar cerrado por inventario'
	                                    WHEN D.rt_stat = 6 THEN '6-Retencion no concluida cerrado por inventario'
	                                    ELSE 'Estado No listado.'
                                    END AS estatus_rt,
                ...
                using (OdbcConnection connection = new OdbcConnection(DL.Connection.GetConnectionStringGen(mode)))
                {
                    ...
                    string query = $@"SELECT
                                    ...
                                    CASE
	                                    ...
	                                    WHEN D.rt_stat = 3 THEN '3-ASN Generado'
	                                    WHEN D.rt_stat = 4 THEN '4-Interfaz Retención para Genesix generada'
	                                    WHEN D.rt_stat = 5 THEN '5-Evento sin registrar cerrado por inventario'
	                                    WHEN D.rt_stat = 6 THEN '6-Retencion no concluida cerrado por inventario'
	                                    ELSE 'Estado No listado.'
                                    END AS estatus_rt,
                                    CASE WHEN (E.des_mot IS NULL)
        ...
        public static async Task<ML.Result> GetDetails(string ord_rel, string pto_alm, string mode)
        {
            ML.Result result = new ML.Result();
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    Uri uri = new Uri($@"{DL.ApiOracle.GetEndpoint(pto_alm, mode)}".Replace("{########}", ord_rel));
        ...
        public static async Task<ML.Result> Confirmation(ML.BaseControl.Confirmation confirmation, string pto_alm, string mode)
        {
            ML.Result result = new ML.Result();
            try
            {
                ...
                ML.Result resultGetDetails = await GetDetails(confirmation.OrdRel, pto_alm, mode);
                if (!resultGetDetails.Correct || resultGetDetails.Object == null)
                {
                    if(resultGetDetails.Object == null)
                    {
                        throw new Exception($@"Oracle no regreso datos de {confirmation.OrdRel}");
                    }
                    else
                    {
                ...
                ML.Result resultBuildData = BuildData(orderDetails, confirmation, pto_alm, mode);
                if (!resultBuildData.Correct)
                {
                    throw new Exception(resultBuildData.Message);
                }
        ...
        private static ML.Result BuildData(List<OrderResult> orderDetails, ML.BaseControl.Confirmation confirmation, string pto_alm, string mode)
        {
            ...
            try
            {
                ...
                ML.Result resultGetOracleCode = GetOracleCode(pto_alm, mode);
                if (!resultGetOracleCode.Correct)
                {
                    throw new Exception(resultGetOracleCode.Message);
                }

                inter.Header.Facility = (string)resultGetOracleCode.Object;
                ...
                using (OdbcConnection connection = new OdbcConnection(DL.Connection.GetConnectionStringGen(mode)))
                {
                    ...
                    using (OdbcCommand cmd = new OdbcCommand(query, connection))
                    {
                        using (OdbcDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                string originCode = reader.GetString(0).Trim();
        ...
        public static async Task<ML.Result> MultiConfirmationsOk(string user, string shipment, string pto_alm, string mode)
        {
            ML.Result result = new ML.Result();
            try
            {

                ML.Result resultGetConfirmations = await GetConfirmationsOK(user, shipment, pto_alm, mode);
                ...
                foreach(ML.BaseControl.Confirmation confirmation in confirmationList)
                {
                    ...
                    ML.Result resultConfirmationOK = await ConfirmationOK(confirmation, pto_alm, mode);
        ...
        private static async Task<ML.Result> ConfirmationOK(ML.BaseControl.Confirmation confirmation, string pto_alm, string mode)
        {
            ML.Result result = new ML.Result();
            try
            {
